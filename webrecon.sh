...
# Thanks to Gr1mmie for his initial work on this https://github.com/Gr1mmie/sumrecon
    ...
    if [ ! -x "$(find / -type f -wholename '/usr/bin/eyewitness')" ];then
    ...
    ######## AMASS SETTINGS (Amass Script or default (non-script) #########
    
    #amass enum -config /usr/share/amass/config.ini -d $url | tee -a $url/recon/final1.txt
    amass enum -d $url | tee -a $url/recon/final1.txt
    #######################################################################
    curl -s https://certspotter.com/api/v0/certs\?domain\=$url | jq '.[].dns_names[]' | sed 's/\"//g' | sed 's/\*\.//g' | sort -u >> $url/recon/final1.txt
    ...
    for domain in $(cat ~/$url/recon/httprobe/alive.txt);do
        ...
        whatweb --info-plugins -t 50 -v $domain >> $url/recon/whatweb/$domain/plugins.txt; sleep 5
        echo "[*] Running whatweb on $domain $(date +'%Y-%m-%d %T')"
        whatweb -t 50 -v https://$domain >> $url/recon/whatweb/$domain/output.txt; sleep 5
    done
    ...
    eyewitness --web -f $url/recon/httprobe/alive.txt -d $url/recon/eyewitness --resolve --no-prompt --timeout 15
